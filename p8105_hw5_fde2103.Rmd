...
Focusing on mu = 1, 2, 3, 4, 5, and 6, my interpretation is that, since power decreases as effect size decreases, positive results are less likely to be "true" positives as effect size decreases. In other words, when the true mu is lower, the estimates that are able to reach the threshold of significance are quite high relative to the true mean, causing an inflation of the average estimate when we only consider the positive results. This inflation that we see dissipates as the true mean increases.
